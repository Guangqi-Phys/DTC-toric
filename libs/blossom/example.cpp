...
void LoadGeomFile(int &node_num, int *&x_array, int *&y_array, char *filename)
{
	...
	while (fgets(LINE, sizeof(LINE) - 1, fp))
	{
		...
		if (sscanf(LINE, "EDGE_WEIGHT_TYPE : EUC_%dD", &DIM) == 1)
		{
			if (DIM != 2)
			{
				printf("only EUC_2D is supported");
				exit(1);
			}
			continue;
		}
...
void SaveMatching(int node_num, GeomPerfectMatching *gpm, char *filename)
{
	...
	for (i = 0; i < node_num; i++)
	{
		...
		if (i < j)
		{
			...
			if (((GeomPerfectMatching::REAL)1 / 2) == 0)
				fprintf(fp, "%d %d %d\n", i, j, len);
			else
				fprintf(fp, "%d %d %f\n", i, j, (double)len);
		}
...
int main(int argc, char *argv[])
{
	...
	for (i = 1; i < argc; i++)
	{
		...
		{
		case 'e':
			if (filename || argv[i][2] || ++i == argc)
				ShowUsage();
			filename = argv[i];
			break;
		case 'g':
			if (geom_filename || argv[i][2] || ++i == argc)
				ShowUsage();
		...
		case 'm':
			...
			if (options.dual_LP_threshold < 0 || options.dual_LP_threshold > 1)
				ShowUsage();
			break;
		case 'd':
			options.dual_greedy_update_option = atoi(&argv[i][2]);
			if (options.dual_greedy_update_option < 1 || options.dual_greedy_update_option > 2)
				ShowUsage();
			break;
		case 'b':
			if (argv[i][2])
		...
		case 'T':
			...
			if (gpm_options.iter_max < 0)
				ShowUsage();
			break;
		case 'w':
			if (save_filename || argv[i][2] || ++i == argc)
				ShowUsage();
	...
	if (!filename && !geom_filename)
		ShowUsage();

	if (filename)
		LoadFile(node_num, edge_num, edges, weights, filename);
	...
	if (!geom_filename)
	{
		...
		printf("cost = %.1f\n", cost);
		if (save_filename)
			SaveMatching(node_num, pm, save_filename);
		delete pm;
	}
	else
	{
		...
		if (filename)
		{
			if (node_num != geom_node_num)
			{
				printf("%s and %s don't match!\n", geom_filename, filename);
				exit(1);
			}
			for (e = 0; e < edge_num; e++)
			{
		...
		gpm->Solve();
		if (save_filename)
			SaveMatching(geom_node_num, gpm, save_filename);
		delete gpm;
	}
